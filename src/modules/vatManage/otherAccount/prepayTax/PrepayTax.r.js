...
const getColumns = ({state}) => [
    {
        title:['项目名称','项目编号'],
        dataIndex:['projectName','projectNum'],
    },
    {
        title:['项目地址','预征项目'],
        dataIndex:['projectAddress','preProject'],
    },
    {
        title:['销售额（含税）','销售额（不含税）'],
        dataIndex:['amountWithTax','amountWithoutTax'],
        render:(text,record)=><div>
                 <p style={{marginBottom:'0px',lineHeight:'1.5',color:'#000'}}>{fMoney(record.amountWithTax)}</p>
                 <p style={{marginBottom:'0px',lineHeight:'1.5',color:'#ccc'}}>{fMoney(record.amountWithoutTax)}</p>
               </div>,
